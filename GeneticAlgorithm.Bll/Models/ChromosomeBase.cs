            ...
            if (genes.Length > 0)
            {
                ...
                if (genesToBeReplacedLength > availableSpaceLength)
                {
                    throw new ArgumentException(
                        $"The number of genes to be replaced is greater than available space, there is {availableSpaceLength} genes between the index {startIndex} and the end of chromosome, but there is {genesToBeReplacedLength} genes to be replaced.");
                }
